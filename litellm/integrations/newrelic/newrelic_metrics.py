...
class NewRelicMetricsLogger(CustomBatchLogger):
    ...
    async def _drain_with_retry(self) -> None:
        ...
        of racing it, so the terminal clear can never wipe a record another
        concurrent drain is still responsible for. Each pass retries the whole
        current queue with backoff, so a record appended mid-drain is picked up
        by a later pass; only after ``_MAX_DRAIN_PASSES`` against a permanently
        failing destination is the remainder cleared, under ``flush_lock`` and
    ...
    async def flush_queue(self) -> None:
        async with self.flush_lock:
            ...
            if self.log_queue:
                ...
                await self.async_send_batch(window_start=window_start)

    async def async_send_batch(self, window_start: float | None = None) -> None:
        if not self.log_queue:
            return
        ...
        payload: Final = build_metric_payload(
            ...
            window_start=window_start if window_start is not None else self.last_flush_time,
            now=time.time(),
        )

        # ``AsyncHTTPHandler.post`` raises ``HTTPStatusError`` on any non-2xx, so a
        # 4xx never returns a response here; read the status off the raised error
        # to keep the client-error path (drop) distinct from 5xx/network (requeue).
        try:
            status = (
        ...
        if 400 <= status < 500:
            if status == 403:
                ...
                verbose_logger.warning(
                    "New Relic Metrics: %s from Metric API, dropping %s records.",
                    status,
                    len(batch_to_send),
                )
            return
