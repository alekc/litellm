    ...
    if test is True:
        ...
        if os.getenv("DATABASE_URL", None) is not None or os.getenv("DIRECT_URL", None) is not None:
            ...
            try:
                ...
                # The reader pool is a real pool against the same configured cap,
                # so it gets the same params. Anything the operator pinned on the
                # replica URL wins, unlike the writer where the config is applied
                # on top.
