...
def _get_tiktoken_count_function(encode_length: Callable[[str], int]) -> TokenCounterFunction:
    def count_tokens(text: str) -> int:
        return sum(
            encode_length(text[start : start + TIKTOKEN_ENCODE_CHUNK_SIZE_CHARS])
            for start in range(0, len(text), TIKTOKEN_ENCODE_CHUNK_SIZE_CHARS)
        )
