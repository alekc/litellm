...
@pytest.mark.asyncio
async def test_record_appended_mid_drain_is_never_stranded():
    """A record a callback appends while a drain is running is picked up by a
    later pass: against a healthy destination it is delivered, and either way
    the queue never ends with a stranded record."""
    logger = _make_logger()
    logger.stop()
    ...
    async def _capture(url, headers=None, content=None, **kw):
        ...
        # Inject the concurrent append during the first send, mid-drain.
        if late_record not in logger.log_queue and _record(model="late-model") not in posted:
    ...
    assert logger.log_queue == [], "nothing stranded: the mid-drain append was drained too"
...
async def test_post_stop_drains_are_serialized():
    ...
    logger.flush_queue = blocking_flush
